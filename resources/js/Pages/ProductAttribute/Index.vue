<template>
    <Head head-key="title" title="Ürün Tipi Seçenekleri" />

    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            
            <PageTitleWithLanguage :routeName="'product-attributes.index'" :title="'Ürün Tipi Seçenek'"/>
            ...
            <div class="card">
                ...
                <div class="card-header px-4">
                    <div class="card-toolbar w-100 justify-content-between gap-2">
                        <NewProductAttribute :product-types="props.product_types.filter((x) => x.language == $page.props.current_language)" :language="$page.props.current_language"/>
                    </div>
                </div>

                <div class="card-body p-0">

                    <DataTable :value="props.product_attributes.data" stripedRows dataKey="id" tableStyle="min-width: 50rem">
                            ...
                            <template #body="{ data }">
                                <div class="hstack gap-2">
                                    <div v-for="(lang,key) in $page.props.languages.active.filter((l) => l != data.language)" :key="key">
                                        <NewProductAttribute :button-class="'btn fw-bold btn-sm btn-icon btn-light-danger text-uppercase'" :button-text="lang" :product-types="props.product_types.filter((x) => x.language == lang)" :language="lang" :from-attribute="data" v-if="!hasConnectedLanguage(data,lang)" v-tooltip.top="'Oluştur'"/>
                        ...
                        <Column class="text-end">
                            <template #body="{ data }">
                                <div class="d-flex flex-nowrap justify-content-end">
                                    ...
                                    <DeletePrompt :uri="route('product-attributes.delete',[data.id,{language:$page.props.current_language}])" :model="data"/>
                                </div>
                            </template>
                        </Column>
                    </DataTable>
                ...
                <div class="card-footer">
                    <Pagination :links="props.product_attributes.links" :only="['product_attributes','flash']"/>
                </div>
            </div>
    ...
    <EditProductAttribute :item="selectedAttribute" :product-types="props.product_types.filter((x) => x.language == selectedAttribute.language)" :on-hide="resetSelectedAttribute" v-if="selectedAttribute"/>
...
import NewProductAttribute from "./NewProductAttribute";
...
const hasConnectedLanguage = (content,lang) => {
    ...
    let data = null;
    const dfilter = content.connected_product_attributes.filter((x) => x.language == lang);
