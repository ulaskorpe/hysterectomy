<template>
    <Head head-key="title" title="Kampanyalar çöp kutusu" />
        ...
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            ...
            <div  class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">
                    Kampanyalar çöp kutusu
                </h1>
            </div>

            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <Link :href="route('campaigns.index')" class="btn fw-bold btn-sm btn-secondary d-flex align-items-center">
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            ...
            <div class="card">
                <div class="card-header px-4" v-if="selectedItems.length === 0">
                    <Filter :filters="props.filters" :uri="route('campaigns.trash')"/>
                </div>
                <div class="card-header px-4" v-if="selectedItems.length > 0">
                    <h5 class="card-title fs-6"><i class="bi bi-check2-circle me-2 fs-2"></i>{{ selectedItems.length }}</h5>
                    <div class="card-toolbar">
                        <RestoreBulkPrompt :uri="route('campaigns.bulk.restore')" :items="selectedItems" :restore-done="resetSelectedItems"/>
                        <DestroyBulkPrompt :uri="route('campaigns.bulk.destroy')" :items="selectedItems" :delete-done="resetSelectedItems"/>
                    </div>
                </div>
                        ...
                        <Column class="text-end">
                            <template #body="{ data }">
                                <div class="d-flex flex-nowrap justify-content-end">
                                    <RestorePrompt :restore-done="resetSelectedItems" :uri="route('campaigns.restore',data.id)" :model="data"/>
                                    <DestroyPrompt :delete-done="resetSelectedItems" :uri="route('campaigns.destroy',data.id)" :model="data"/>
                                </div>
                            </template>
                        </Column>
                    </DataTable>
