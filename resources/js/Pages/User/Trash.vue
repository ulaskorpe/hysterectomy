<template>
    <Head head-key="title" title="Sistem kullanıcı çöp kutusu" />
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            ...
            <div class="card">
                <div class="card-header px-4" v-if="selectedItems.length === 0">
                    <Filter :filters="props.filters" :uri="route('users.trash')"/>
                </div>
                <div class="card-header px-4" v-if="selectedItems.length > 0">
                    <h5 class="card-title fs-6"><i class="bi bi-check2-circle me-2 fs-2"></i>{{ selectedItems.length }}</h5>
                    <div class="card-toolbar">
                        <RestoreBulkPrompt :uri="route('users.bulk.restore')" :items="selectedItems" :restore-done="resetSelectedItems"/>
                        <DestroyBulkPrompt :uri="route('users.bulk.destroy')" :items="selectedItems" :delete-done="resetSelectedItems"/>
                    </div>
                </div>
                        ...
                        <Column class="text-end">
                            <template #body="{ data }">
                                <div class="d-flex flex-nowrap justify-content-end">
                                    <RestorePrompt :restore-done="resetSelectedItems" :uri="route('users.restore',data.id)" :model="data"/>
                                    <DestroyPrompt :delete-done="resetSelectedItems" :uri="route('users.destroy',data.id)" :model="data"/>
                                </div>
                            </template>
                        </Column>
                    </DataTable>
