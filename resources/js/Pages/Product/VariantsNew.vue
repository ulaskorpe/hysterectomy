<template>
    ...
    <div>
        ...
        <Dialog v-model:visible="dialogVisible" modal header="Varyasyon Ekle" :style="{ width: '50vw' }" :breakpoints="{ '960px': '75vw', '641px': '95vw' }">
            <form @submit.prevent="create" v-if="props.product.product_type.option_group && props.product.product_type.option_group.options">
...
const form = useForm({
    option_values:props.product.product_type.option_group.options,
    media_objects:{
        mainImage:null
    },
    stock:null,
...
const create = () => {
    form.post(route('product-variants.store',props.product.id),{
        onSuccess: () => {
            form.reset();
